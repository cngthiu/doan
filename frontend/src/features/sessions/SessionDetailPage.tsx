...
export function SessionDetailPage() {
  ...
  const canUpload = canManage && can(permissions.mediaUpload)
  ...
  const load = async () => {
    ...
    try {
      ...
      const [seatItems, candidateItems] = await Promise.all([
        getSeats(detail.room_id),
        canManage ? getCandidates({ pageSize: 50 }).then((page) => page.items) : Promise.resolve([]),
      ])
      ...
      setCandidates(mergeCandidates(candidateItems, detail.assignments.map((item) => item.candidate)))
  ...
  useEffect(() => {
    ...
    return () => { active = false }
  }, [canManage, debouncedSearch, session?.id])
  ...
  return <div className="page-stack">
    ...
    <SessionReadinessPanel session={session} actions={<>
      {editable && <Link className="secondary-button link-button" to="/monitoring">Mở màn hình giám sát</Link>}
    ...
    <section className="card media-section"><div className="section-heading"><div><h2>Video nguồn</h2><p>Video MP4/H.264 được phát bằng trình phát của trình duyệt.</p></div></div>
      ...
      {editable && canUpload && <div className="video-upload-panel"><h3>{session.video ? 'Thay video nguồn' : 'Tải video lên'}</h3><VideoUpload onUploaded={attachVideo} /></div>}
    </section>
    <section className="card" id="assignments"><div className="section-heading"><div><h2>Thí sinh và chỗ ngồi</h2><p>Mỗi thí sinh và chỗ ngồi chỉ được sử dụng một lần trong phiên thi.</p></div>{editable && <button className="primary-button" type="button" disabled={saving || !dirtyAssignments} onClick={() => void save()}>{saving ? 'Đang lưu…' : 'Lưu phân công'}</button>}</div>
      {editable && <><label htmlFor="assignment-search">Tìm thí sinh</label><input id="assignment-search" className="candidate-filter" placeholder="Nhập mã hoặc họ tên…" value={search} onChange={(event) => setSearch(event.target.value)} />{candidateLoading && <p className="inline-loading">Đang tìm thí sinh…</p>}</>}
      <div className="assignment-list">{seats.map((seat) => {
        const assignment = session.assignments.find((item) => item.seat.id === seat.id)
        return <div className="assignment-row" key={seat.id}><strong>{seat.code}</strong>{editable ? <select value={values[seat.id ?? ''] ?? ''} onChange={(event) => choose(seat.id ?? '', event.target.value)}><option value="">Chưa xếp thí sinh</option>{candidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.candidate_code} — {candidate.full_name}</option>)}</select> : <span className="assignment-value">{assignment ? `${assignment.candidate.candidate_code} — ${assignment.candidate.full_name}` : 'Chưa xếp thí sinh'}</span>}{editable && values[seat.id ?? ''] && <button className="danger-link" type="button" onClick={() => choose(seat.id ?? '', '')}>Bỏ xếp</button>}</div>
      })}</div>
