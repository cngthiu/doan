...
export function MonitoringPage() {
  ...
  return <div className="page-stack monitoring-page">
    ...
    {error && <><ErrorState message={error} onRetry={() => loadSessions(debouncedSessionQuery)} />{errorDetail && <details className="error-detail"><summary>Chi tiết kỹ thuật</summary><code>{errorDetail}</code></details>}</>}
    ...
    {selected && <SessionReadinessPanel session={selected} actions={<Link className="secondary-button link-button" to={`/sessions/${selected.id}`}>Hoàn thiện thiết lập</Link>} />}
